...
from .api_client import KiwiAPIClient
from .response_mapper import KiwiResponseMapper
...
class KiwiProvider(BaseFlightProvider):
    def __init__(
        ...
        self._api_client = KiwiAPIClient(api_key, http_client)
        self._mapper = KiwiResponseMapper()
    ...
    async def _perform_search(self, criteria: SearchCriteria) -> list[Flight]:
        results = await self._api_client.search_flights(criteria)
